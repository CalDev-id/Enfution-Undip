...
const CCAdmin = ({ trx, filter, search, info, status }) => {
    ...
    const urlFilter = "/dashboard/coaching-clinic?periode=";
    ...
    const getBuktiBayar = () => {
        fetch(`/getPaymentSlipCS?id=${id}`, {
            ...
            .then((data) => {
                setbuktiBayar(`/uploads/cc_payment_slip/${data.bukti}`);
            });
    };
        ...
        return trx.map((t) => {
            return (
                <tr className="text-center" key={no}>
                    <th>{no++}</th>
                    <td>{t.cc_participant.full_name}</td>
